...
const HirePaymentModal: React.FC<HirePaymentModalProps> = ({ employeeName, fee, onClose, onSubmit }) => {
  const [step, setStep] = useState<'selectMethod' | 'otherMethodsInfo' | 'enterCode'>('selectMethod');
  const [confirmationCode, setConfirmationCode] = useState('');
  const [error, setError] = useState('');

  const handlePayPalClick = () => {
    const itemName = `Placement Fee for ${employeeName}`;
    const paypalUrl = `https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=${paymentDetails.paypal.email}&currency_code=USD&amount=${fee}&item_name=${encodeURIComponent(itemName)}`;
      ...
      case 'enterCode':
        return (
          <>
            ...
            <p className="text-slate-600 mb-4">Enter the code you received to finalize the hire of <strong>{employeeName}</strong>.</p>
...
export default HirePaymentModal;